...
// Example: /sys/dev/char/506:0 -> ../../devices/pci0000:15/0000:15:01.0/0000:16:00.0/0000:17:02.0/0000:19:00.0/0000:1a:00.0/0000:1b:00.0/vfio-dev/vfio0
func getBdfFromVFIODev(major uint32, minor uint32) (string, error) {
	...
	if len(parts) < 3 {
		return "", fmt.Errorf("path has fewer than 3 components: %s", cleaned)
	}
	// Check if the final component *resembles* a BDF by containing ':' and '.'
	if !strings.Contains(cleaned, ":") || !strings.Contains(cleaned, ".") {
		return "", fmt.Errorf("no BDF found in path: %s", cleaned)
	}
...
func GetDeviceFromVFIODev(device config.DeviceInfo) ([]*config.VFIODev, error) {
	...
	// device major:minor entries in /sys/dev/char/major:minor
