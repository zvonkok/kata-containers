...
pub async fn handle_cdi_devices(
    ...
    if let Some(container_type) = spec.annotations().as_ref().unwrap().get("io.katacontainers.pkg.oci.container_type") {
    ...
    Err(anyhow!("failed to inject devices after CDI timeout of {} seconds", TIMEOUT_LIMIT))
}
