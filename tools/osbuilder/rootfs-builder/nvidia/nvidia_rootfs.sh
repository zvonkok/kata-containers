...
setup_nvidia-kdev() 
{
	...
	local TARBALL="${BUILD_DIR}/kata-static-${TARGET}.tar.xz"
...
setup_nvidia_gpu_rootfs_stage_one()
{
	...
	for component in "nvidia-gpu-admin-tools" "nvidia-dcgm-exporter" "nvidia-kdev"; do
		if [ ! -e "${BUILD_DIR}/kata-static-${component}.tar.xz" ]; then
			setup_${component}
		fi
	done
	...
	mount -t proc /proc ./proc
        ...
        chroot . /bin/bash -c "/nvidia_chroot.sh $(uname -r) ${run_file_name} ${run_fm_file_name} ${ARCH} ${rootfs_type}"
...
chisseled_dcgm() 
{
	...
	tar xvf "${BUILD_DIR}"/kata-static-nvidia-dcgm-exporter.tar.xz -C .
}
...
chisseled_gpu() 
{
	...
	tar xvf "${BUILD_DIR}"/kata-static-nvidia-gpu-admin-tools.tar.xz -C .
	...
	libdir="lib64"
	cp -a "${stage_one}"/${libdir}/ld-linux-x86-64.so.2* ${libdir}/.

	tar xvf "${BUILD_DIR}"/kata-static-nvidia-kdev.tar.xz -C .
...
chisseled_init() 
{
	...
	mkdir -p dev etc proc run sys tmp usr var lib/modules \
...
setup_nvidia_gpu_rootfs_stage_two() 
{
	...
	mkdir  "${stage_one}"
	mv "${stage_two}"/* "${stage_one}"/.

	if echo "$NVIDIA_STACK_COMPONENTS" | grep -q '\<debug\>'; then
    		DEBUG="true"
	fi
	...
	echo "nvidia: before compressing: $(du -s)"
	compress_rootfs
	echo "nvidia: after compressing: $(du -s)"
