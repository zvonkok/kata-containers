...
EOF
	...
	if [ "${AGENT_POLICY}" = "yes" ]; then
		AGENT_TARBALL="kata-static-agent-opa.tar.xz"
	else
		AGENT_TARBALL="kata-static-agent.tar.xz"
	fi

	if [ ! -e "${BUILDDIR}/${AGENT_TARBALL}" ]; then
		build_agent_from_source
	else
		tar -C "${ROOTFS_DIR}" -xvf "${BUILDDIR}/${AGENT_TARBALL}"
	fi
